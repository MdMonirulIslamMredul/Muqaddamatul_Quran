                ...
                <div class="row">
                    <div class="col-md-12 text-center">
                        ...
                        <ul class="breadcrumb white justify-content-center">
                            ...
                            @if ($subcategory->bookCategory)
                                <li>
                                    <a href="{{ route('category.book', $subcategory->bookCategory->id) }}" class="text-white">
                                        @if (session()->get('language') == 'bangla')
                                            {{ $subcategory->bookCategory->category_name_ban ?? $subcategory->bookCategory->category_name }}
                                        @elseif (session()->get('language') == 'arabic')
                                            {{ $subcategory->bookCategory->category_name_ab ?? $subcategory->bookCategory->category_name }}
                                        @else
                                            {{ $subcategory->bookCategory->category_name }}
                                        @endif
                                    </a>
                                </li>
                            @endif
                ...
                <div class="col-lg-9 col-md-8">
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-25 pb-15 border-bottom">
                        <div>
                            ...
                            @if ($subcategory->bookCategory)
                                <small class="text-muted">
                                    ...
                                    @if (session()->get('language') == 'bangla')
                                        মূল বিভাগ: {{ $subcategory->bookCategory->category_name_ban ?? $subcategory->bookCategory->category_name }}
                                    @elseif (session()->get('language') == 'arabic')
                                        القسم الأساسي: {{ $subcategory->bookCategory->category_name_ab ?? $subcategory->bookCategory->category_name }}
                                    @else
                                        Category: {{ $subcategory->bookCategory->category_name }}
                                    @endif
                                </small>
                            @endif
                        </div>
                        ...
                        <div class="d-flex gap-2 mt-2 mt-sm-0">
                            @if ($subcategory->bookCategory)
                                <a href="{{ route('category.book', $subcategory->bookCategory->id) }}" class="btn btn-sm btn-outline-success font-12 font-weight-600">
                                    <i class="fa fa-arrow-up me-1"></i>
                                    @if (session()->get('language') == 'bangla') মূল বিভাগে যান @elseif (session()->get('language') == 'arabic') القسم الرئيسي @else Parent Category @endif
                                </a>
                            @endif
                            <a href="{{ route('book.page') }}" class="btn btn-sm btn-outline-secondary font-12 font-weight-600">
                                <i class="fa fa-th-large me-1"></i>
                                @if (session()->get('language') == 'bangla') সকল বই @elseif (session()->get('language') == 'arabic') جميع الكتب @else All Books @endif
                            </a>
                        </div>
                    </div>
                            ...
                            <div class="col-12 py-60 text-center">
                                <div class="p-40 bg-light rounded">
                                    ...
                                    <h4 class="text-secondary font-weight-600">
                                        @if (session()->get('language') == 'bangla')
                                            এই উপ-বিভাগে কোন বই পাওয়া যায়নি
                                        @elseif (session()->get('language') == 'arabic')
                                            لم يتم العثور على كتب في هذا القسم الفرعي
                                        @else
                                            No Books Found In This Subcategory
                                        @endif
                                    </h4>
                            ...
                            @foreach ($categories as $cat)
                                <li class="category-nav-item">
                                    <a href="{{ route('category.book', $cat->id) }}"
                                        class="category-nav-link {{ ($subcategory->category_id == $cat->id) ? 'active' : '' }}">
                                        <span>
                                            ...
                                            @foreach ($cat->bookSubcategories as $sub)
                                                <li>
                                                    <a href="{{ route('subcategory.book', $sub->id) }}"
                                                        class="sub-nav-link {{ ($subcategory->id == $sub->id) ? 'active' : '' }}">
                                                        <span>
